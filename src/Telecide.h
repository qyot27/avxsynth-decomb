			...
			if ((f = fopen(ovr, "r")) != NULL)
			{
				...
				while(fgets(buf, 80, f) != 0)
				{
					...
					if (*d != 0 && *(d+1) == 'p' || *(d+1) == 'c' || *(d+1) == 'n')
					{
						...
						// AMD64: Casting a pointer to an int? tsk tsk.
						// *p++ = (unsigned int) x;
						*((unsigned int**)p) = x;
						// Ugly, but portable; p=p+1 on x86, p=p+2 on x64.
						p += (sizeof(unsigned int*)/sizeof(unsigned int));
...
#if !(defined _AMD64_ || defined AVXSYNTH)
		__asm emms;
#endif
	}
...
//			while (*(p+1) == 'm' && *p != 0 && *p != 0xffffffff)
			// Look like we gotta use this doozy again
			while (*(p+(sizeof(unsigned int*)/sizeof(unsigned int))) == 'm' && *(int**)p != (int*)0 && *(int**)p != (int*)-1)
			{
				free(*(void**)p);
				p += (3+(sizeof(unsigned int*)/sizeof(unsigned int)));
			}
			free(overrides);
		}
	}
