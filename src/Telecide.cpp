...
PVideoFrame Telecide::GetFrame(int frame, IScriptEnvironment* env)
{
	...
	if (overrides_p != NULL)
	{
		...
		/* AMD64: This is pretty dumb, all the (sizeof(unsigned int*)/sizeof(unsigned int))
		*  is there to jump a pointer. On x86 we add 1, on x64 add 2. It looks really
		*  ugly but the compiler should have no trouble thinning it down.
		*/
			// If the frame is in range...
			if (((unsigned int) frame >= *overrides_p) && ((unsigned int) frame <= *(overrides_p+1)))
			{
				// and it's a single specifier. 
				if (*(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))) == 'p' || 
					*(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))) == 'c' || 
					*(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))) == 'n')
				{
					// Get the match specifier and stop parsing.
					switch(*(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))))
					{
				...
				else if (*(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))) == 'b')
				{
					back = *(overrides_p+2);
				}
				else if (*(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))) == 'm')
				{
			...
			// Next override line.
			overrides_p += (3+(sizeof(unsigned int*)/sizeof(unsigned int)));
		}
	}
	...
	if (guide != GUIDE_NONE)
	{
		...
		if (frame >= cycle && PredictHardYUY2(frame, &predicted, &predicted_metric) == true)
		{
			...
			if (chosen != predicted)
			{
				...
				else mismatch = (100.0*abs((float)predicted_metric - lowest))/predicted_metric;
				if (mismatch < gthresh)
				{
	...
	if (overrides_p != NULL)
	{
		while (*overrides_p < 0xffffffff)
		{
			...
			if (((unsigned int) frame >= *overrides_p) && ((unsigned int) frame <= *(overrides_p+1)))
			{
				if (*(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))) == 'v')
				{
	...
	if (overrides_p != NULL)
	{
		while (*overrides_p < 0xffffffff)
		{
			...
			if (((unsigned int) frame >= *overrides_p) && ((unsigned int) frame <= *(overrides_p+1)) &&
				...
				(*(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))) == '+' || 
				 *(overrides_p+2+(sizeof(unsigned int*)/sizeof(unsigned int))) == '-'))
			{
